...
export default function MenuScreen() {
  ...
  const filterOptions = [
    ...
    ...menu.map(s => ({ id: s.id, label: t(`menu.section.${s.id}`, { defaultValue: s.categoryKey }) })),
  ];

  const allSections = menu.map(s => {
    let items = s.items;
    if (search) {
      items = items.filter(item =>
        localName(item.name, lang).toLowerCase().includes(search.toLowerCase()) ||
        localName(item.description, lang).toLowerCase().includes(search.toLowerCase())
      );
    }
    return { id: s.id, title: t(`menu.section.${s.id}`, { defaultValue: s.categoryKey }), data: items, icon: s.icon };
            ...
            <View key={section.id} style={styles.sectionBlock}>
              ...
              <HorizontalFadeScroll>
                {section.data.map(item => (
                  <MenuCard key={item.id} item={item} navigation={navigation} icon={section.icon} />
                ))}
              </HorizontalFadeScroll>
            </View>
          ))
